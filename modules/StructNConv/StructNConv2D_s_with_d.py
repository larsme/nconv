...
class StructNConv2D_s_with_d(torch.nn.Module):
    def __init__(self, init_method='k', mirror_weights=False, 
                 in_channels=1, in_channels_d=1, out_channels=1, groups=1, channel_first=False,
        ...
        self.w_prop = torch.nn.Parameter(data=torch.Tensor(1, self.out_channels, 1, 1))
        if self.in_channels > 1:
            self.channel_weight = torch.nn.Parameter(data=torch.Tensor(self.out_channels, self.in_channels, 1, 1))
        if self.in_channels_d > 1 and self.in_channels_d != self.out_channels:
            self.channel_weight_d = torch.nn.Parameter(data=torch.Tensor(self.out_channels, self.in_channels_d, 1, 1))
        if mirror_weights:
        ...
        if self.init_method == 'x':  # Xavier
            ...
            if self.in_channels_d > 1 and self.in_channels_d != self.out_channels:
                torch.nn.init.xavier_uniform_(self.channel_weight_d) + 1
            ...
            if self.in_channels_d > 1 and self.in_channels_d != self.out_channels:
                torch.nn.init.kaiming_uniform_(self.channel_weight_d)
    ...
    def enforce_limits(self):
        ...
        if self.in_channels_d > 1 and self.in_channels_d != self.out_channels:
            self.channel_weight_d.data = F.softplus(self.channel_weight_d, beta=10)
    ...
    def forward(self, d, cd, s, cs):
        ...
        if self.in_channels_d > 1 and self.in_channels_d != self.out_channels:
            # Normalized Convolution along channel dimensions
            nom = F.conv2d(cd * d, self.channel_weight_d, groups=self.groups)
            denom = F.conv2d(cd, self.channel_weight_d, groups=self.groups)
            d = nom / (denom + self.eps)
            cd = denom / (torch.sum(self.channel_weight_d) + self.eps)
        ...
        if self.in_channels_d == 1 and self.in_channels_d != self.out_channels:
            min_div_max = min_div_max.expand(-1,self.out_channels,-1,-1)
        ...
        if self.stride == 1:
            # postpone if max already used stride, but spatial conv did not
        ...
        nom = F.conv2d(cs * s, self.spatial_weight, groups=self.out_channels, stride=self.stride, padding=self.padding, dilation=self.dilation).squeeze(2)
        denom = F.conv2d(cs, self.spatial_weight, groups=self.out_channels, stride=self.stride, padding=self.padding, dilation=self.dilation).squeeze(2)
        s = nom / (denom + self.eps)
