...
class StructNDeconv2d_gy_with_d(torch.nn.Module):
    ...
    def forward(self, d, cd, s, cs, gx, cgx, gy, cgy, s_prod_roll):
        ...
        nom = F.conv3d(cgy_prop * gy_roll, self.statial_weight, groups=self.in_channels).squeeze(2)
        denom = F.conv3d(cgy_prop, self.statial_weight, groups=self.in_channels).squeeze(2)
