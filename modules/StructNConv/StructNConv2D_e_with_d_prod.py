...
class StructNConv2D_e_with_d(torch.nn.Module):
    ...
    def forward(self, d, cd, s, cs):
        ...
        if self.kernel_size == 3:
            ...
            w_max = d * cd + self.eps
            w_min = cd / (d + self.eps) + self.eps
            w_max_denom = F.avg_pool2d(w_max, kernel_size=3, stride=1, padding=1)
            w_min_denom = F.avg_pool2d(w_min, kernel_size=3, stride=1, padding=1)


            d_min, d_max = F.avg_pool2d(d * w_min, kernel_size=3, stride=1, padding=1) / w_min_denom, F.avg_pool2d(d * w_max, kernel_size=3, stride=1, padding=1) / w_max_denom
            cd_min, cd_max = F.avg_pool2d(cd * w_min, kernel_size=3, stride=1, padding=1) / w_min_denom,  F.avg_pool2d(cd * w_max, kernel_size=3, stride=1, padding=1) / w_max_denom
        ...
        w_min = c_min_div_max / (d_min_div_max + self.eps)
        w_min = w_min / (w_min.sum(2, keepdim=True) + self.eps)

        s_from_d = (w_min * d_min_div_max).sum(2)
        cs_from_d = (w_min * c_min_div_max).sum(dim=2)
