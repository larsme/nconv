...
class StructNConv2d_gy_with_d(torch.nn.Module):
    ...
    def forward(self, d, cd, s, cs, gx, cgx, gy, cgy, s_prod_roll):
        ...
        if self.channel_first:
            ...
            nom = F.conv3d(cgy_prop * gy_roll, self.channel_weight, self.groups)
            denom = F.conv3d(cgy_prop, self.channel_weight, self.groups)
            gy_channel = (nom / (denom+self.eps))
            cgy_channel = (denom / torch.sum(self.spatial_weight))

            # Normalized Convolution along spatial dimensions
            nom = F.conv3d(cgy_channel * gy_channel, self.channel_weight, groups=self.in_channels, stride=self.stride,
                           padding=self.padding, dilation=self.dilation).squeeze(2)
            denom = F.conv3d(cgy_channel, self.channel_weight, groups=self.in_channels, stride=self.stride,
                             padding=self.padding, dilation=self.dilation).squeeze(2)
            gy = nom / (denom+self.eps)
            cgy = denom / torch.sum(self.channel_weight)
        else:
            # Normalized Convolution along spatial dimensions
            nom = F.conv3d(cgy_prop * gy_roll, self.statial_weight, groups=self.in_channels, stride=self.stride,
                           padding=self.padding, dilation=self.dilation).squeeze(2)
            denom = F.conv3d(cgy_prop, self.statial_weight, groups=self.in_channels, stride=self.stride,
                             padding=self.padding, dilation=self.dilation).squeeze(2)
