...
###### Training Set ######
    if 'train' in sets:
        ...
        image_datasets['train'] = KittiDepthDataset(train_kitti_depth_dir, setname='train',
                                                    ...
                                                    desired_image_width=1216, desired_image_height=352, dtype=params["dtype"])
...
###### Selected Validation set ######
    if 'selval' in sets:
        selval_kitti_depth_dir = os.path.join(
            kitti_depth_dir, 'val_selection_cropped')

        image_datasets['selval'] = KittiDepthDataset(selval_kitti_depth_dir, setname='selval',
                                                     norm_factor=norm_factor, invert_depth=invert_depth,
                                                     load_rgb=load_rgb, rgb_dir=kitti_rgb_dir,
                                                     resize=False, lidar_padding=lidar_padding, dtype=params["dtype"])
...
###### Selected test set ######
    if 'test' in sets:
        test_kitti_depth_dir = os.path.join(
            kitti_depth_dir, 'test_depth_completion_anonymous')
